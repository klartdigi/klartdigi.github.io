...
$(function () {
  /** Scroll to sections */
  $('#header .navbar-nav li a, #home a').click(function (e) {
    $('#header .navbar-nav li a.active').removeClass('active');
    var href = e.currentTarget.getAttribute('href');
    $("#header .navbar-nav li a[href=\"".concat(href, "\"]")).addClass('active');
    var newPosition = $(href).offset();
    $('html, body').stop().animate({
      scrollTop: newPosition.top - 50
    }, 500);
    e.preventDefault();
  });
  /** Client auto scroll */

  var animateScroll = function animateScroll(line, speed) {
    var children = line.children();
    setInterval(function () {
      var scrollWidth = Math.round(line[0].scrollWidth);
      var scrollLeft = line.scrollLeft();
      var currentScrollLeft = Math.round(scrollLeft + Math.round(line.width()));

      if (currentScrollLeft > scrollWidth - 1) {
        children.each(function (index, element) {
          line.append("<p>".concat(element.innerHTML, "</p>"));
        });
      }
  ...
  var secureElement;
  var parkingElement;
  var quickSetupElement; //  Get X and Y of elements
  ...
  var animateTile = function animateTile(element) {
    element.css('transition', '0.5s all ease-out');
    element.css('transform', 'translate(0px, 0px)');
    element.css('transform-origin', 'center');
  };

  var animateTileReset = function animateTileReset(element, position) {
    var diffX = centerElement.width() / 2 + centerElement.offset().left - position.left - element[0].getBBox().width / 2;
    var diffY = centerElement.height() / 2 + centerElement.offset().top - position.top - element[0].getBBox().height / 2;
    element.css('transition', '0.5s all ease-out');
    element.css('transform', "translate(".concat(diffX, "px, ").concat(diffY, "px)"));
    element.css('transform-origin', 'center');
  ...
  var animateElements = function animateElements() {
    ...
    setTimeout(function () {
      animateTile(ticketElement, ticketPosition);
    }, offset + 0);
    setTimeout(function () {
      animateTile(inventoryElement, inventoryPosition);
    }, offset + 50);
    setTimeout(function () {
      animateTile(salesElement, salesPosition);
    }, offset + 100);
    setTimeout(function () {
      animateTile(employeeElement, employeePosition);
    }, offset + 150);
    setTimeout(function () {
      animateTile(supportElement, supportPosition);
    }, offset + 200);
    setTimeout(function () {
      animateTile(secureElement, securePosition);
    }, offset + 250);
    setTimeout(function () {
      animateTile(parkingElement, parkingPosition);
    }, offset + 300);
    setTimeout(function () {
      animateTile(quickSetupElement, quickSetupPosition);
    }, offset + 350);
  };

  var animateElementsReset = function animateElementsReset() {
    var logo = $('#features .svg-wrapper svg #logo');
    setTimeout(function () {
      logo.css('animation', 'none');
    }, 0);
    setTimeout(function () {
      animateTileReset(ticketElement, ticketPosition);
    }, 0);
    setTimeout(function () {
      animateTileReset(inventoryElement, inventoryPosition);
    }, 50);
    setTimeout(function () {
      animateTileReset(salesElement, salesPosition);
    }, 100);
    setTimeout(function () {
      animateTileReset(employeeElement, employeePosition);
    }, 150);
    setTimeout(function () {
      animateTileReset(supportElement, supportPosition);
    }, 200);
    setTimeout(function () {
      animateTileReset(secureElement, securePosition);
    }, 250);
    setTimeout(function () {
      animateTileReset(parkingElement, parkingPosition);
    }, 300);
    setTimeout(function () {
      animateTileReset(quickSetupElement, quickSetupPosition);
    }, 350);
  };

  var toggleSVG = function toggleSVG() {
